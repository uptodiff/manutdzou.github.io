...
现在我们来介绍 softmax 回归算法的代价函数。在下面的公式中$\left( 1\{ y^{(i)} = j\}  - p(y^{(i)} = j | x^{(i)}) \right)$是，$1\left{\cdot\right}$,是示性函数，其取值规则为：$1 \left\{ 值为真的表达式 \textstyle \right\}=1$， $1 \left\{ 值为假的表达式 \textstyle \right\}=0$。举例来说，表达式 $1 \left\{2+2=4 \right\}$ 的值为1 ，$1 \left\{1+1=5 \right\}$的值为 0。我们的代价函数为：
