...
集群是由所有的ps和worker组成。在运行tensorflow脚本的时候，集群之间每台机器需要知道谁是谁，也就是相互之间的角色，比如谁是参数服务器（ps）,谁是计算服务器（worker）。只有当所有的机器配置挂载完成以后程序才会开始运行。参数服务器只负责共享参数的计算更新，计算服务器负责每个单独流图的计算。

这是另一个例子，可见其实框架都一样

如下定义了pc-01为参数服务器，pc-02，pc-03,pc-04为计算服务器。 ps和workers叫做jobs,每个jobs包含若干个tasks
